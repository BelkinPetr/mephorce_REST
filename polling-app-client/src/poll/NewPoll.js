...
import {MAX_CHOICES, POLL_QUESTION_MAX_LENGTH, POLL_CHOICE_MAX_LENGTH, POLL_NAME_MAX_LENGTH} from '../constants';
import './NewPoll.css';
...
class NewPoll extends Component {
    ...
    handleSubmit(event) {
        ...
        createPoll(pollData)
            ...
            if(error.status === 401) {
                this.props.handleLogout('/login', 'error', 'Вы покинули платформу!Для того, чтобы создать проект, авторизуйтесь.');
            } else {
                notification.error({
                    message: 'Mephorce',
                    description: error.message || 'Упс.Что-то пошло не так. Повторите попытку.'
                });
            }
        });
    }
    ...
    validateQuestion = (questionText) => {
        if(questionText.length === 0) {
            return {
                ...
                errorMsg: 'Пожалуйста введите описание проекта'
            }
        } else if (questionText.length > POLL_QUESTION_MAX_LENGTH) {
            return {
                validateStatus: 'error',
                errorMsg: `Описание слишком длинное (Максимально разрешенное количество символов ${POLL_QUESTION_MAX_LENGTH})`
            }
        } else {
            return {
    ...
    validateName = (nameText) => {
        if(nameText.length === 0) {
            return {
                validateStatus: 'error',
                errorMsg: 'Пожалуйста введите название проекта'
            }
        } else if (nameText.length > POLL_NAME_MAX_LENGTH) {
            return {
                validateStatus: 'error',
                errorMsg: `Название слишком длинное (Максимально разрешенное количество символов ${POLL_NAME_MAX_LENGTH})`
            }
        } else {
            return {
                validateStatus: 'success',
    ...
    render() {
        ...
        return (
            <div className="new-poll-container">
                <h1 className="page-title">Создание проекта</h1>
                    ...
                    <Form onSubmit={this.handleSubmit} className="create-poll-form">
                        ...
                        <FormItem className="poll-form-row">
                            <Button type="dashed" onClick={this.addChoice} disabled={this.state.choices.length === MAX_CHOICES}>
                                <Icon type="plus" /> Добавить тематику
                            </Button>
                        </FormItem>
                        <FormItem className="poll-form-row">
                            <Col xs={24} sm={4}>
                        ...
                        <FormItem className="poll-form-row">
                            <Button type="primary"
                                    ...
                                    className="create-poll-form-button">Создать</Button>
                        </FormItem>
                    </Form>
                </div>
            </div>
...
function PollChoice(props) {
    return (
        <FormItem validateStatus={props.choice.validateStatus}
            ...
            <Input
                placeholder = {'Тематика проекта ' + (props.choiceNumber + 1)}
                size="large"
